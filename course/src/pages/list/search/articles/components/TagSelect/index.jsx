...
const TagSelectOption = ({ children, checked, onChange, value }) => (
  <CheckableTag checked={!!checked} key={value} onChange={(state) => onChange && onChange(value, state)}>
    {children}
  </CheckableTag>
)
...
class TagSelect extends Component {
  ...
  getAllTags() {
    ...
    const checkedTags = childrenArray.filter((child) => this.isTagSelectOption(child)).map((child) => child.props.value)
    return checkedTags || []
  }
